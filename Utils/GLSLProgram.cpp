...
GLSLProgram::GLSLProgram(const char *vsource, const char *fsource)
{
	...
	std::string vpath = basePath + vsource;

	char* vs = textFileRead(vpath.c_str());

	std::string fpath = basePath + fsource;
	char* fs = textFileRead(fpath.c_str());

	mProg = compileProgram(vs, 0, fs);
                         ...
                         GLenum gsInput, GLenum gsOutput)
{
	...
	std::string gpath = basePath + gsource;
	char* gs = textFileRead(gpath.c_str());

	std::string fpath = basePath + fsource;
	char* fs = textFileRead(fpath.c_str());
	
	mProg = compileProgram(vs, gs, fs, gsInput, gsOutput);
